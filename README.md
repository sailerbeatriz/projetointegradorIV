...
A aplicação não é publicada em nuvem: a distribuição é pelo GitHub e a execução é local.
...
| Seção | Função |
|---|---|
| Visão Geral | Série temporal agregada do Estado e estatísticas principais |
| Município | Série individual com alerta de limiar ajustável |
| Agrupamento | Perfil dos 4 grupos (gráficos em facetas, legíveis para daltonismo) |
| Alerta Precoce | Curva de cobertura × taxa de alerta por limiar de escore z |
| Comparativo | Dois municípios lado a lado |

Paleta de cores: verde (#3fa78a), terracota (#d97757) e azul (#4a7fb5), validada para daltonismo.

## Metodologia

### Preparação (script 01)

- Remove a categoria “Ignorado”, que não tem população de referência (1.363 registros, um por data)
- Mantém as 1.247 variações diárias negativas decorrentes de retificações (772 em casos, 475 em óbitos); as análises usam a média móvel de 7 dias
...
Veja `requirements.txt` para as versões exatas.
...
Claude (IA): desenvolvimento da interface Streamlit, estilização visual, acessibilidade (validação de cores para daltonismo), configuração de tema e CSS, script de geração das figuras (`04_figuras.py`) e script de verificação (`05_verificacoes.py`).
