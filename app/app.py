...
# Paleta única do painel. Azul para o dado, laranja para o alerta — a cor de
# estado nunca é reaproveitada como cor de série.
...
if pagina == "Visão geral":
    ...
    fig = px.area(estado, x="datahora", y=coluna, color_discrete_sequence=[AZUL])
    fig.update_traces(line=dict(width=2), fillcolor="rgba(42,120,214,0.12)")
...
# ══════════════════════════════ MUNICÍPIO ══════════════════════════════
    ...
    for data in d.loc[alerta.fillna(False), "datahora"]:
        fig.add_vline(x=data, line=dict(color=LARANJA, width=1), opacity=0.18)
    fig.add_trace(go.Scatter(
        x=d["datahora"], y=d["casos_mm7d"], mode="lines",
        line=dict(color=AZUL, width=2), name="casos (média móvel de 7 dias)",
    ))
...
# ══════════════════════════════ AGRUPAMENTO ══════════════════════════════
    ...
    with c2:
        ...
        fig = px.imshow(
            ...
            color_continuous_scale=[[0, AZUL], [0.5, "#eef0f2"], [1, LARANJA]],
            zmin=-float(abs(perfil.values).max()), zmax=float(abs(perfil.values).max()),
            labels=dict(color="escore z"),
        )
        fig.update_layout(template="simple_white", height=380, font=dict(color=TINTA, size=12),
    ...
    fig = px.scatter(
        ...
        color=filtrado["grupo"].astype(str), size="populacao", size_max=26,
        hover_name="nome_munic", opacity=0.75,
        color_discrete_sequence=CORES_GRUPO, labels={"color": "Grupo"},
    )
    fig.update_traces(marker=dict(line=dict(width=0.7, color="white")))
    st.plotly_chart(eixo_limpo(fig, "Letalidade aparente (%)"), width="stretch")
...
# ══════════════════════════════ ALERTA PRECOCE ══════════════════════════════
    ...
    fig.add_trace(go.Scatter(x=tab["taxa_alerta"], y=tab["cobertura"], mode="lines+markers+text",
                             ...
                             line=dict(color=AZUL, width=2), marker=dict(size=10),
                             name="limiar"))
